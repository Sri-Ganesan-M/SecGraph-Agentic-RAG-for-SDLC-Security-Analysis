...
from langchain.agents import Tool # To define tools for the agent
...
# --- Paths to your built knowledge bases ---
# IMPORTANT: Ensure these paths match the EXACT directories created by your KB builder script.
# Corrected path for requirements_phase_security_kb (lowercase with underscore)
...
# --- Helper Function to Load and Configure Retriever ---
...
# --- Common LLM and Retriever Setup ---
llm_for_tools = ChatOllama(model=LLM_MODEL, temperature=0.2) # Lower temp for more factual output

# Load retrievers once when the module is imported
# This will print "Successfully loaded..." for each existing KB, or "Error: Vector DB not found..."
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            ("system",
             ...
             2.  Why it's a concern (briefly, e.g., "allows unauthorized database access").
...
if __name__ == "__main__":
    ...
    # # --- Test Design Tool ---
    # print("--- Testing DesignSecurityAnalyzer ---")
    # design_input = """
    # Our API gateway will expose all internal microservices. Authentication will be handled at the individual microservice level. Data will be transmitted via HTTP.
    # """
    # design_suggestion = analyze_design_security(design_input.strip(), "Microservices")
    # print("\nSecurity Suggestions for Design:\n", design_suggestion)
    # print("\n" + "="*80 + "\n")

    # # --- Test Development Tool - Code Snippet ---
    # print("--- Testing DevelopmentSecurityAnalyzer (Code) ---")
    # code_input = """
    # import flask
    ...
    #     file.save('/tmp/' + file.filename) # Directly saving user-provided filename
    #     return 'File uploaded'
    # """
    # dev_suggestion = analyze_development_security(code_input.strip(), "Python")
    # print("\nSecurity Suggestions for Code Snippet:\n", dev_suggestion)
    # print("\n" + "="*80 + "\n")

    # # --- Test Testing Tool ---
    # print("--- Testing TestingSecurityAnalyzer ---")
    # test_input = """
    # Our current penetration test scope only covers public-facing web pages. We haven't tested any backend APIs or internal admin interfaces.
    # """
    # test_suggestion = get_testing_security_suggestions(test_input.strip(), "Penetration Test")
    # print("\nSecurity Suggestions for Testing:\n", test_suggestion)
    # print("\n" + "="*80 + "\n")

    # # --- Test Deployment Tool ---
    # print("--- Testing DeploymentSecurityAnalyzer ---")
    # deploy_input = """
    # We are deploying our application to an AWS EC2 instance. We use security groups but SSH port (22) is open to 0.0.0.0/0. Database credentials are hardcoded in the deployment script.
    # """
    # deploy_suggestion = analyze_deployment_security(deploy_input.strip(), "AWS")
    # print("\nSecurity Suggestions for Deployment:\n", deploy_suggestion)
    # print("\n" + "="*80 + "\n")